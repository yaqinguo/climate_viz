...
t_data%>%
  ...
  geom_text(x=1880,y=0.8,hjust=0,
            label=glue("Global temperatures increasced by over {max_t_diff} \u00B0C since {min(t_data$year)}"),
                       color="white")+
  #theme_void()+
  ...
  scale_fill_stepsn(colors = c("darkblue","white","darkred"),
                       ...
                       limits=c(min(t_data$t_diff),max(t_data$t_diff)),
                    n.breaks=9)+
  scale_y_continuous(breaks = seq(-0.6,1.2,0.3),limits = c(-0.6,1.2))+
